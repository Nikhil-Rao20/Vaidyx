...
Vaidyx keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/vaidyx-backup` tool
...
```bash
# Create a snapshot → backups/vaidyx-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/vaidyx-backup snapshot

# List existing snapshots (most recent first)
./scripts/vaidyx-backup list

# Check a tarball's integrity without extracting it
./scripts/vaidyx-backup verify backups/vaidyx-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/vaidyx-backup restore backups/vaidyx-backup-20260101-120000.tar.gz --yes
```
...
./scripts/vaidyx-backup snapshot --out /mnt/nas/vaidyx-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/vaidyx-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/vaidyx && ./scripts/vaidyx-backup snapshot --out "/mnt/nas/vaidyx-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `vaidyx-backup` therefore does not capture the Docker
