...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _EMAIL_LOCAL_DRAFT_PREFIX = 'vaidyx.email.replyDraft.v1:';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__vaidyxActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'vaidyx-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'vaidyx-doc-minimized-' + sessionId;
  ...
  let _vaidyxAttachMenu = null;

  function _closeVaidyxAttachMenu() {
    if (_vaidyxAttachMenu) {
      _vaidyxAttachMenu.remove();
      _vaidyxAttachMenu = null;
    }
  ...
  function _attachMenuOutsideClick(e) {
    if (_vaidyxAttachMenu && !_vaidyxAttachMenu.contains(e.target)) _closeVaidyxAttachMenu();
  }

  function _attachMenuEscape(e) {
    if (e.key !== 'Escape') return;
    _closeVaidyxAttachMenu();
  }

  function _positionVaidyxAttachMenu(anchor, menu) {
  ...
  function _vaidyxAttachLabel(item, kind) {
    if (kind === 'gallery') {
  ...
  async function _stageVaidyxAttachment(kind, id) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-vaidyx`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function _stageVaidyxZip(items) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-vaidyx-zip`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  function _afterVaidyxAttachmentsAdded(count, label) {
    _renderComposeAttachments();
    clearTimeout(_autoSaveDebounce);
    _autoSaveDebounce = setTimeout(() => { saveDocument({ silent: true }); }, 800);
  ...
  async function _attachVaidyxItem(kind, id, label, opts = {}) {
    try {
      const data = await _stageVaidyxAttachment(kind, id);
      if (!data) return;
      _afterVaidyxAttachmentsAdded(1, label || data.filename);
      if (!opts.keepOpen) _closeVaidyxAttachMenu();
    } catch (err) {
      console.error('Failed to attach Vaidyx item:', err);
      if (uiModule) uiModule.showError('Failed to attach from Vaidyx');
    }
  }

  function _selectedVaidyxAttachRows(menu) {
    return Array.from(menu?.querySelectorAll?.('.email-vaidyx-attach-row.is-selected') || []);
  }

  function _syncVaidyxAttachSelection(menu) {
    const selected = _selectedVaidyxAttachRows(menu);
    const bar = menu?.querySelector?.('.email-vaidyx-attach-actions');
    const count = menu?.querySelector?.('.email-vaidyx-attach-count');
    const attachBtn = menu?.querySelector?.('.email-vaidyx-attach-selected');
  ...
  async function _attachSelectedVaidyxItems(menu) {
    const rows = _selectedVaidyxAttachRows(menu);
    if (!rows.length) return;
    const btn = menu.querySelector('.email-vaidyx-attach-selected');
    if (btn) {
      btn.disabled = true;
    ...
    try {
      ...
      if (zip) {
        await _stageVaidyxZip(items);
        added = 1;
      } else {
        for (const item of items) {
          await _stageVaidyxAttachment(item.kind, item.id);
          added += 1;
        }
      }
      _afterVaidyxAttachmentsAdded(added, zip ? 'vaidyx-attachments.zip' : undefined);
      _closeVaidyxAttachMenu();
    } catch (err) {
      console.error('Failed to attach selected Vaidyx items:', err);
      if (uiModule) uiModule.showError(added ? `Attached ${added}, then failed` : 'Failed to attach from Vaidyx');
      _renderComposeAttachments();
    } finally {
      if (btn) {
  ...
  async function _loadVaidyxAttachItems(menu, kind) {
    const list = menu.querySelector('.email-vaidyx-attach-list');
    if (!list) return;
    ...
    const q = (menu.querySelector('.email-vaidyx-attach-search')?.value || '').trim();
    try {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="email-vaidyx-attach-empty">${q ? 'No matches' : `No ${kind === 'gallery' ? 'images' : 'documents'}`}</div>`;
        _syncVaidyxAttachSelection(menu);
        return;
      }
      list.innerHTML = '';
      for (const item of items) {
        const label = _vaidyxAttachLabel(item, kind);
        const row = document.createElement('button');
        row.type = 'button';
        row.className = `email-vaidyx-attach-row ${kind === 'gallery' ? 'is-gallery' : ''}`;
        row.dataset.id = item.id || '';
        row.dataset.kind = kind;
        if (kind === 'gallery') {
          const src = item.url ? `${API_BASE}${item.url}` : '';
          row.innerHTML = `
            <span class="email-vaidyx-attach-dot" aria-hidden="true"></span>
            <span class="email-vaidyx-attach-thumb">${src ? `<img src="${_escHtml(src)}" alt="">` : ''}</span>
            <span class="email-vaidyx-attach-main">
              <span class="email-vaidyx-attach-title">${_escHtml(label)}</span>
              <span class="email-vaidyx-attach-meta">${_escHtml(item.filename || 'image')}</span>
            </span>
          `;
        } else {
          row.innerHTML = `
            <span class="email-vaidyx-attach-dot" aria-hidden="true"></span>
            <span class="email-vaidyx-attach-icon">${langIcon(item.language || 'text', 14, { style: 'opacity:0.8;' })}</span>
            <span class="email-vaidyx-attach-main">
              <span class="email-vaidyx-attach-title">${_escHtml(label)}</span>
              <span class="email-vaidyx-attach-meta">${_escHtml(item.language || 'text')}</span>
            </span>
          `;
        }
        ...
        row.addEventListener('dblclick', () => _attachVaidyxItem(kind, item.id, label, { keepOpen: false }));
        list.appendChild(row);
      }
      _syncVaidyxAttachSelection(menu);
    } catch (err) {
      console.error('Failed to load Vaidyx attach items:', err);
      list.innerHTML = '<div class="email-vaidyx-attach-empty">Could not load</div>';
    }
  }
  ...
  function _showComposeAttachMenu(anchor) {
    ...
    menu.className = 'email-vaidyx-attach-menu';
    menu.innerHTML = `
      <button type="button" class="email-vaidyx-attach-local">
      ...
      <div class="email-vaidyx-attach-tabs">
      ...
      <label class="email-vaidyx-attach-search-wrap">
        ...
        <input type="search" class="email-vaidyx-attach-search" placeholder="Search attachments">
      </label>
      <div class="email-vaidyx-attach-list"></div>
      <div class="email-vaidyx-attach-actions">
        <span class="email-vaidyx-attach-count"></span>
        <button type="button" class="email-vaidyx-attach-selected" disabled>
    ...
    _vaidyxAttachMenu = menu;
    _positionVaidyxAttachMenu(anchor, menu);
    menu.querySelector('.email-vaidyx-attach-local')?.addEventListener('click', () => {
      _closeVaidyxAttachMenu();
      document.getElementById('doc-email-file-input')?.click();
    });
    menu.querySelectorAll('[data-ody-attach-kind]').forEach(btn => {
      btn.addEventListener('click', () => _loadVaidyxAttachItems(menu, btn.dataset.odyAttachKind));
    });
    let attachSearchTimer = null;
    menu.querySelector('.email-vaidyx-attach-search')?.addEventListener('input', () => {
      clearTimeout(attachSearchTimer);
      attachSearchTimer = setTimeout(() => {
        _loadVaidyxAttachItems(menu, menu.dataset.odyAttachKind || 'document');
      }, 220);
    });
    menu.querySelector('.email-vaidyx-attach-selected')?.addEventListener('click', () => _attachSelectedVaidyxItems(menu));
    setTimeout(() => {
      document.addEventListener('click', _attachMenuOutsideClick, true);
      document.addEventListener('keydown', _attachMenuEscape, true);
    }, 0);
    _loadVaidyxAttachItems(menu, 'document');
  }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__vaidyxActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const _AI_REPLY_CONTEXT_STORE_PREFIX = 'vaidyx:email-ai-reply-context:v1:';
  ...
  async function _aiReply(opts = {}) {
    ...
    const sourceUid = document.getElementById('doc-email-source-uid')?.value?.trim() || '';
    const sourceFolder = document.getElementById('doc-email-source-folder')?.value?.trim() || 'INBOX';
    const sourceAccountId = docs.get(activeDocId)?.sourceEmailAccountId || window.__vaidyxActiveEmailAccount || '';
  ...
  export function openPanel() {
    ...
    pane.innerHTML = `
      ...
      <div id="doc-email-actions" class="doc-email-actions" style="display:none">
        ...
        <div class="email-send-split">
          <button type="button" id="doc-email-send-btn" class="email-send-btn email-send-main" title="Send email (Ctrl+Enter)" onpointerdown="window.vaidyxEmailSendIntent&&window.vaidyxEmailSendIntent(event)" onmousedown="window.vaidyxEmailSendIntent&&window.vaidyxEmailSendIntent(event)" onclick="window.vaidyxEmailSendIntent&&window.vaidyxEmailSendIntent(event)"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Send</button>
          <button type="button" id="doc-email-send-caret" class="email-send-btn email-send-caret" title="More send options" aria-haspopup="true" aria-expanded="false" onpointerdown="window.vaidyxEmailCaretIntent&&window.vaidyxEmailCaretIntent(event)" onmousedown="window.vaidyxEmailCaretIntent&&window.vaidyxEmailCaretIntent(event)"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"/></svg></button>
    ...
    const handleSendIntent = (e) => {
      if (e && e.__vaidyxEmailSendHandled) return;
      ...
      if (e) {
        e.preventDefault();
        e.stopPropagation();
        e.__vaidyxEmailSendHandled = true;
      }
    ...
    window.vaidyxEmailSendIntent = handleSendIntent;
    ...
    const handleCaretIntent = (e) => {
      if (e && e.__vaidyxEmailCaretHandled) return;
      const now = Date.now();
      if (e && e.type === 'click' && now - lastCaretToggleAt < 350) {
        e.preventDefault();
        e.stopPropagation();
        e.__vaidyxEmailCaretHandled = true;
        return;
      }
      const rawTarget = e && e.target;
      const target = rawTarget && rawTarget.nodeType === Node.TEXT_NODE ? rawTarget.parentElement : rawTarget;
    ...
    window.vaidyxEmailCaretIntent = handleCaretIntent;
    ...
    let _fontIdx = parseInt(localStorage.getItem('vaidyx-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('vaidyx-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  function initMdToolbar() {
    ...
    toolbar.addEventListener('mousedown', (e) => {
      if (e.target.closest('[data-md], .md-dd-toggle, .md-toolbar-attach-btn')) e.preventDefault();
    });
  ...
  const _DOC_RECENTS_KEY = 'vaidyx-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('vaidyx-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('vaidyx-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('vaidyx-suggestions-' + docId);
      if (!raw) return;
  ...
  function runDocument() {
    ...
    outputPanel.innerHTML = '<pre class="doc-run-error">Unsupported language for preview.</pre>';
  ...
  function exportAsHtml() {
    ...
    if (lang === 'markdown' && markdownModule?.mdToHtml) {
      ...
      body = '<pre style="white-space:pre-wrap;font-size:12px;font-family:\'Elms Sans\',monospace;">' +
        text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') + '</pre>';
    }
    const title = docs.get(activeDocId)?.title || 'document';
    const html = `<!DOCTYPE html>\n<html><head><meta charset="utf-8"><title>${title.replace(/</g,'&lt;')}</title></head><body style="max-width:800px;margin:40px auto;font-family:'Elms Sans',sans-serif;line-height:1.6;padding:0 20px;">\n${body}\n</body></html>`;
  ...
  async function exportAsPdf() {
    ...
    if (lang === 'markdown' && markdownModule?.mdToHtml) {
      ...
      html = '<pre style="white-space:pre-wrap;font-size:11px;font-family:\'Elms Sans\',monospace;color:#000;background:#fff;">' +
        text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') + '</pre>';
    }
    const container = document.createElement('div');
    container.style.cssText = "padding:20px;font-family:'Elms Sans',sans-serif;font-size:12px;color:#000;background:#fff;line-height:1.6;";
