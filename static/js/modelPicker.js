...
const RECENT_KEY = 'vaidyx-model-recent';
const FAVORITES_KEY = 'vaidyx-model-favorites';
const RECENT_MAX = 5;
...
async function _ensureDefaultPendingChat() {
  ...
  try {
    let dc = null;
    try {
      dc = window.__vaidyxDefaultChat || null;
    } catch (_) {}
...
function _initModelPickerDropdown() {
  ...
  async function _refreshLocalProbe() {
    try {
      if (window.__vaidyxChatBusy || Date.now() < (window.__vaidyxChatBusyUntil || 0)) return;
    } catch (_) {}
    const now = Date.now();
  ...
  const _PROVIDER_NAMES = {
    ...
    'nvidia': 'NVIDIA',
    'perceptron': 'Perceptron', 'poolside': 'Poolside',
  ...
  const _PROVIDER_ALIAS = {
    ...
    '~google': 'google', '~moonshotai': 'moonshotai',
  };
  ...
  const _collapsedProviders = new Set(_loadList('vaidyx-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('vaidyx-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
...
async function _pick(m) {
    _defaultPendingSeq++;
    try {
      window.__vaidyxLastPickedRoute = {
        model: m.mid || '',
        endpoint_url: m.url || '',
    ...
    try { window.__vaidyxModelSwitchPromise = switchPromise; } catch (_) {}
    const finishSwitch = () => {
      try {
        if (switchDone) switchDone();
        if (window.__vaidyxModelSwitchPromise === switchPromise) delete window.__vaidyxModelSwitchPromise;
      } catch (_) {}
    };
    ...
    try { document.dispatchEvent(new CustomEvent('vaidyx:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('vaidyx:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  if (!modelId && !currentSessionId && !_pendingChat && _deps.setPendingChat) {
    ...
    try {
      cachedDefault = window.__vaidyxDefaultChat || null;
    } catch (_) {}
    if (!cachedDefault || !cachedDefault.endpoint_url || !cachedDefault.model) {
      try {
        cachedDefault = JSON.parse(localStorage.getItem('vaidyx-default-chat-cache') || 'null');
      } catch (_) {}
    }
    if (cachedDefault && cachedDefault.endpoint_url && cachedDefault.model) {
  ...
  // SECURITY: deliberately NOT auto-injecting `vaidyx-model-favorites[0]`
