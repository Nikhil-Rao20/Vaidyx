...
export const THEMES = {
  vaidyx: { bg:'#f8fafb', fg:'#1e293b', panel:'#ffffff', border:'#e2e8f0', red:'#0891b2',
                advanced: { brandColor: '#0891b2', sendBtnBg: '#0891b2', sendBtnHover: '#0e7490',
                            sidebarBg: '#f1f5f9', sectionAccent: '#0891b2',
                            userBubbleBg: '#f0f9ff', aiBubbleBg: '#ffffff',
                            bubbleBorder: '#e2e8f0',
                            inputBg: '#ffffff', inputBorder: '#e2e8f0',
                            codeBg: '#f8fafc', codeFg: '#334155',
                            accentPrimary: '#0891b2', accentError: '#dc2626' } },
  'vaidyx-dark': { bg:'#0a1628', fg:'#e0f0f8', panel:'#0d1f3c', border:'#1e293b', red:'#01c0c8',
                advanced: { brandColor: '#01c0c8', sendBtnBg: '#01c0c8', sendBtnHover: '#0195b1',
                            sidebarBg: '#060f1e', userBubbleBg: '#0d2a4a', aiBubbleBg: '#0d1f3c',
                            inputBg: '#0d1f3c', inputBorder: '#1e293b' } },
};

const DEFAULT_THEME = 'vaidyx';
const LS_KEY = 'vaidyx-theme';
const CUSTOM_THEMES_KEY = 'vaidyx-custom-themes';

const FONT_MAP = {
  mono: "'Fira Code', 'Elms Sans', monospace",
  sans: "'Elms Sans', system-ui, -apple-system, sans-serif",
  serif: "Georgia, 'Times New Roman', 'Elms Sans', serif",
...
function deriveSyntaxColors(colors) {
  ...
  const [redH, redS, redL] = hexToHSL(colors.red || '#01c0c8');
  const isDark = bgL < 50;
...
const ADV_KEYS = [
  ...
  { key: 'brandColor',         css: '--brand-color',       label: 'Vaidyx Logo',    group: 'Sidebar' },
...
export function applyColors(colors) {
  ...
  const isDarkBg = bgRgb ? (bgRgb.r * 0.299 + bgRgb.g * 0.587 + bgRgb.b * 0.114) < 128 : false;
  document.documentElement.classList.toggle('dark-bg', isDarkBg);

  document.querySelectorAll('img[src*="vaidyx-icon"]').forEach(img => {
    img.src = isDarkBg
      ? img.src.replace('vaidyx-icon.svg', 'vaidyx-icon-white.svg')
      : img.src.replace('vaidyx-icon-white.svg', 'vaidyx-icon.svg');
  });
  document.querySelectorAll('img[src*="vaidyx-wordmark"]').forEach(img => {
    if (isDarkBg) {
      img.src = img.src.replace('vaidyx-wordmark.svg', 'vaidyx-wordmark-white.svg');
    } else {
      img.src = img.src.replace('vaidyx-wordmark-white.svg', 'vaidyx-wordmark.svg');
    }
  });
  ...
  _updateFavicon(colors.red || '#01c0c8', isDarkBg);
}
...
function _updateFavicon(fg, isDark) {
  ...
  if (routeShape) {
    ...
    href = isDark ? '/static/vaidyx-icon-white.svg' : '/static/vaidyx-icon.svg';
  }
...
const UI_SCALE_KEY = 'vaidyx-ui-scale';
const DEFAULT_UI_SCALE = '100';
const UI_SCALE_STEPS = [85, 90, 95, 100, 110, 125, 150];
...
export function applyUiScale(scale) {
  ...
  if (pct === 100 || isNaN(pct)) {
    ...
    const zoomVal = pct / 100;
    document.documentElement.style.setProperty('--ui-zoom', zoomVal);
    document.documentElement.setAttribute('data-ui-zoom', s);
  }
}

function _scaleToSliderIndex(scale) {
  const pct = parseInt(scale, 10) || 100;
  const idx = UI_SCALE_STEPS.indexOf(pct);
  return idx >= 0 ? idx : 3;
}

function _sliderIndexToScale(index) {
  return String(UI_SCALE_STEPS[index] || 100);
}

function _updateScaleTicks(activeIndex) {
  const ticks = document.querySelectorAll('.theme-scale-ticks span');
...
export function getSaved() {
  ...
  if (obj && !THEMES[obj.name]) {
    const ct = _loadCustomThemes();
    if (!ct[obj.name]) obj.name = DEFAULT_THEME;
  }
  return obj;
}
...
export function initThemeUI() {
  ...
  grid.innerHTML = Object.entries(THEMES).map(([name, c]) => `
    <div class="theme-swatch${name === activeName ? ' active' : ''}" data-theme="${name}">
      ...
      ${name === 'vaidyx-dark' ? 'Vaidyx Dark' : name === 'vaidyx' ? 'Vaidyx' : name}
    </div>
  `).join('');
  ...
  // Use a one-time bind flag to avoid duplicate listeners.
  ...
  const textSizeSlider = document.getElementById('theme-text-size-slider');
  const textSizeValueLabel = document.getElementById('theme-scale-value');
  if (textSizeSlider) {
    ...
    textSizeSlider.value = _scaleToSliderIndex(initScale);
    if (textSizeValueLabel) textSizeValueLabel.textContent = initScale + '%';
    _updateScaleTicks(_scaleToSliderIndex(initScale));
    textSizeSlider.addEventListener('input', () => {
      const scale = _sliderIndexToScale(parseInt(textSizeSlider.value, 10));
      applyUiScale(scale);
      if (textSizeValueLabel) textSizeValueLabel.textContent = scale + '%';
      _updateScaleTicks(parseInt(textSizeSlider.value, 10));
      try { localStorage.setItem(UI_SCALE_KEY, scale); } catch (e) {}
    });
  }
  if (patternSelect) {
  ...
  if (harmonyAccentEl && _harmonyHex) {
    _harmonyHex.textContent = harmonyAccentEl.value || '#01c0c8';
  ...
  if (exportBtnEl) {
    ...
    newExp.addEventListener('click', () => {
      ...
      a.href = url;
      a.download = 'vaidyx_' + (obj.name || 'theme') + '.json';
      a.click();
...
async function _initWithSync() {
  ...
  if (!getSaved()) {
    ...
    if (serverTheme && serverTheme.colors) {
      if (!THEMES[serverTheme.name]) {
        const ct = _loadCustomThemes();
        if (!ct[serverTheme.name]) serverTheme.name = DEFAULT_THEME;
      }
