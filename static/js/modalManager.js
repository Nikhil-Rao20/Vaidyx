...
const _rememberedDockKey = (id) => `vaidyx-modal-remembered-dock-${id}`;
...
const _DOCK_STORAGE_KEY = 'vaidyx.mobileDockState.v1';
...
function _renderDock() {
  ...
  // see the attribute wiped on every re-render — most visibly after a
  // chain drag, when chips are at body level and get swept by the next
  // render.
...
export function register(id, { restoreFn, closeFn, railBtnId, sidebarBtnId, label, icon } = {}) {
  ...
  // already-open modals. The various tool open() functions (cookbook,
...
export function injectMinimizeButton(modal, modalId) {
  ...
  if (closeBtn) {
    // The close button may carry its own left margin meant to separate it
    // from the title when it stood alone. Now that `_` sits to its left,
    // that margin becomes a stray gap
