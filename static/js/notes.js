...
let _searchQuery = '';
let _viewMode = (typeof localStorage !== 'undefined' && localStorage.getItem('vaidyx-notes-view')) || 'list'; // 'list' or 'grid'
...
const REMINDER_FIRED_KEY = 'vaidyx-notes-reminder-fired';
...
const REMINDER_GLOWED_KEY = 'vaidyx-notes-reminder-glowed';
...
const REMINDER_PENDING_HIGHLIGHT_KEY = 'vaidyx-notes-reminder-pending-highlight';
const REMINDER_ACTIVE_HIGHLIGHT_KEY = 'vaidyx-notes-reminder-active-highlight';
...
const REMINDER_DISMISSED_AT_KEY = 'vaidyx-notes-reminder-dismissed-at';
const NOTES_FIRST_OPEN_HINT_KEY = 'vaidyx-notes-first-open-hint-v1';
...
function _bringNotesToFront(pane = document.getElementById('notes-pane')) {
  ...
  try {
    window.dispatchEvent(new CustomEvent('vaidyx:modal-opened', {
...
export function openPanel() {
  ...
  if (viewBtn) {
    ...
    viewBtn.addEventListener('click', () => {
      ...
      try { localStorage.setItem('vaidyx-notes-view', _viewMode); } catch {}
      pane.classList.toggle('notes-view-grid', _viewMode === 'grid');
      _setViewLabel();
      requestAnimationFrame(() => _applyMasonry(document.querySelector('#notes-pane .notes-pane-body')));
    });
...
const _DRAFT_PREFIX = 'vaidyx-note-draft-';
...
function _wireCanvas(container, initialImageUrl) {
  ...
  const _openTextInput = (e) => {
    ...
    input.style.cssText = [
      ...
      `font:${sizeCss}px 'Elms Sans', sans-serif`,
      `color:${color}`,
      'background:#ffffff',
    ...
    const commit = () => {
      ...
      ctx.font = `${logicalSize}px 'Elms Sans', sans-serif`;
