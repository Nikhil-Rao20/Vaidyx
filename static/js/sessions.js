...
let _sortMode = Storage.get('vaidyx-session-sort') || 'active'; // default to last active
...
function _shouldPreserveStartupComposer(msgInput) {
  ...
  if (window.__vaidyxComposerUserEdited) return true;
...
function _installHistoryPager(id, pageInfo, modelName) {
  ...
  const loadOlder = async () => {
    ...
    const anchor = box.querySelector('.msg, .agent-thread');
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Vaidyx Chat';
...
const FOLDER_STATE_KEY = 'vaidyx-folder-state';
const FOLDER_ORDER_KEY = 'vaidyx-folder-order';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  try {
    ...
    currentSessionId = id;
    try { window.__vaidyxLastSelectedSessionId = id; } catch (_) {}
    ...
    // URL hash — the user complained that coming back to Vaidyx kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Vaidyx Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('vaidyx-doc-open-' + id) === '1';
...
export function createDirectChat(url, modelId, endpointId, opts = {}) {
  ...
  currentSessionId = null;
  try { window.__vaidyxLastSelectedSessionId = ''; } catch (_) {}
  Storage.remove('lastSessionId');
  history.replaceState(null, '', window.location.pathname);
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('vaidyx-session-sort', mode);
  else Storage.remove('vaidyx-session-sort');
  renderSessionList();
}
