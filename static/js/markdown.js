...
export function svgifyEmoji(html, opts) {
  ...
  // Walk the HTML to convert real Unicode emoji into monochrome SVG icons.
  if (!_EMOJI_RE.test(html)) return html;
...
function initMermaid() {
  if (!window.mermaid || window.__vaidyxMermaidReady) return;
  window.mermaid.initialize({ startOnLoad: false, theme: 'dark', securityLevel: 'loose' });
  window.__vaidyxMermaidReady = true;
}
window.vaidyxInitMermaid = initMermaid;
initMermaid();
...
const THINK_EXPANDED_KEY = 'vaidyx-thinking-expanded';
