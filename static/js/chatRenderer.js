...
const _ACTION_RECENTS_KEY = 'vaidyx-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'vaidyx-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Vaidyx';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Vaidyx' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
    if (role === 'assistant' && (metadata?.research || metadata?.research_clarification)) {
      _roleText += ' (Research)';
    }
    if (metadata?.character_name && role !== 'user' && !isSlash && !isCompacted) {
