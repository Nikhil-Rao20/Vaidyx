// ============================================
// Vaidyx UI — Main Application Orchestrator
...
import documentModule from './js/document.js?v=20260809cleanup1';
import searchChatModule from './js/search-chat.js';
import { makeWindowDraggable } from './js/windowDrag.js';
import markdownModule from './js/markdown.js';
import chatRenderer from './js/chatRenderer.js?v=20260809cleanup1';
...
import settingsModule from './js/settings.js?v=20260809cleanup1';
// Eagerly bind unified minimize/restore behavior across all tool modals.
import './js/modalManager.js?v=20260809cleanup1';
...
function _isForegroundChatBusy() {
  ...
  return !!window.__vaidyxChatBusy
    || Date.now() < (window.__vaidyxChatBusyUntil || 0)
...
function _submitMobileQueuedInput(input) {
  ...
  window.__vaidyxQueueStreamingSubmit = now;
...
try {
  const cachedDefaultChat = JSON.parse(localStorage.getItem('vaidyx-default-chat-cache') || 'null');
  if (cachedDefaultChat && cachedDefaultChat.endpoint_url && cachedDefaultChat.model) {
    _defaultChat = cachedDefaultChat;
    window.__vaidyxDefaultChat = cachedDefaultChat;
  }
} catch (_) {}
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try {
        window.__vaidyxDefaultChat = d;
        localStorage.setItem('vaidyx-default-chat-cache', JSON.stringify(d));
      } catch (_) {}
      return d;
    }
  } catch (_) {}
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Vaidyx Chat';
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      ...
      // so a window stacked on another only dismisses the top one, not both.
      ...
      const dynamicModals = ['library-modal', 'archive-modal', 'doclib-modal', 'tasks-modal'];
  ...
  // only displays the rail when `.sidebar.hidden` is set. Used by /notes
  // route opener so that fullscreen view keeps the rail
  ...
  // Expose so notes close can call this without needing to import
  // app.js directly.
  ...
  if (_opener) window._vaidyxRouteOpener = _opener;
  ...
  // editor. stopPropagation so it doesn't also fire the row's open-library click.
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('vaidyx-model-sort', mode);
  ...
  // Prefetch settings to apply UI tweaks early
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  function applyModeToToggles(mode) {
    ...
    window.__vaidyxSetChatMode = setMode;
  ...
  (function initPlanToggle() {
    ...
    window.__vaidyxSetPlanMode = (active) => setPlanMode(active, { silent: true });
    const statusToggle = el('plan-mode-status-toggle');
    if (btn) {
      btn.addEventListener('click', () => {
        const st = loadToggleState();
    ...
    if (msgInput && !msgInput._vaidyxPlanTabToggle) {
      msgInput._vaidyxPlanTabToggle = true;
    ...
    if (chatBar && !chatBar._vaidyxPlanSwipeToggle) {
      chatBar._vaidyxPlanSwipeToggle = true;
      let touchStartX = 0;
      let touchStartY = 0;
      let touchStartAt = 0;
  ...
  const SPLASH_COUNT_KEY = 'vaidyx-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  // External modules dispatch this when their overflow state changes
  ...
  (function initModelPickerResponsive() {
    ...
    const TOOLBAR_HIDE_WIDTH = 160;
	    ...
	    function setComposerPlaceholder(width) {
	      ...
	      textarea.setAttribute('placeholder', width < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message Vaidyx...');
	    }

	    if (_isMobile && textarea && !textarea._vaidyxPlanPlaceholderHint) {
	      textarea._vaidyxPlanPlaceholderHint = true;
	      setInterval(() => {
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__vaidyxSetChatMode === 'function') {
          window.__vaidyxSetChatMode('chat');
        } else {
        ...
        if (typeof window.__vaidyxSetChatMode === 'function') {
          window.__vaidyxSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'vaidyx-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'vaidyx-toolbar-visibility';
    try {
  ...
  (function initUIVisibility() {
    ...
    // The rename-session modal lacks a per-module makeWindowDraggable call.
    // Wire it onto the shared helper, drag-only, to match its old behavior.
    try {
      ['rename-session-modal'].forEach((id) => {
        const m = document.getElementById(id);
        if (!m) return;
  ...
  (function initModalMinimize() {
    const SKIP_IDS = new Set(['styled-confirm-overlay']);
  ...
  initSidebarLayout(Storage, {
    ...
    sessionModule, el, _defaultChat, _syncResearchIndicator
  });
  ...
  (function initTabSwipe() {
    ...
    document.addEventListener('touchend', (e) => {
      ...
      for (const [barSel, tabSel] of SYSTEMS) {
        ...
        const host = bar.closest('.modal, #notes-pane, .admin-card') || bar.parentElement;
  ...
  if (textarea) {
    ...
    window.addEventListener('vaidyx:chat-busy-change', () => _syncMobileEnterKeyHint(textarea));
      ...
      if (_shouldQueueFromMobileEnter(e, textarea) || (e.key === 'Enter' && !e.shiftKey && !e.isComposing && !isMobile)) {
        ...
        if (form) {
          if (_isForegroundChatBusy() && textarea.value && textarea.value.trim()) {
            ...
            window.__vaidyxQueueStreamingSubmit = Date.now();
          }
          _submitChatFormDirect(form);
        }
      }
...
// ============================================
function startVaidyxApp() {
  tasksModule?.startNotificationPolling?.();
  if (window.__vaidyxAppStarted) return;
  window.__vaidyxAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__vaidyxChatBusyUntil = Math.max(window.__vaidyxChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('vaidyx-doc-open-' + _curSession) === '1') {
  ...
  // Sync the contextual rail icons. Tool launchers (calendar/cookbook/
  // research/tasks/archive/memory/notes/theme) are now always-visible
  ...
  window._vaidyxBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      if (sendBtn.dataset.mode === 'streaming') {
        if (hasText) window.__vaidyxQueueStreamingSubmit = Date.now();
        handleSubmit(e);
        return;
      }
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._vaidyxRouteOpener) {
          try { window._vaidyxRouteOpener(); } catch (_) {}
          window._vaidyxRouteOpener = null;
        }
      });
  } else {
  ...
  const runNonCriticalStartup = (fn, delay = 4000) => {
    let tries = 0;
    const run = () => {
      const busy = !!window.__vaidyxChatBusy
        || Date.now() < (window.__vaidyxChatBusyUntil || 0)
        || !!document.querySelector('.send-btn[data-mode="streaming"], .send-btn.send-pending');
      if (busy && tries < 12) {
        tries += 1;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startVaidyxApp, { once: true });
} else {
  startVaidyxApp();
}
