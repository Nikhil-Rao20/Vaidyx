...
**Environment flags:** `ODYSSEUS_DISABLE_MCP` (disable all), `ODYSSEUS_BROWSER_MCP_REQUIRE_CACHE` (require cached package), `ODYSSEUS_BROWSER_EXECUTABLE`,
`ODYSSEUS_BROWSER_ISOLATED` (default on), `ODYSSEUS_BROWSER_NO_SANDBOX` (default on).
...
| `DATA_DIR`            | `ODYSSEUS_DATA_DIR` or default| Root for all persisted data      |
...
calls: `ODYSSEUS_INTERNAL_BASE` > `APP_PORT` > fallback `http://127.0.0.1:7000`.
