# Odysseus Frontend & UI Documentation
...
Odysseus uses **vanilla JavaScript ES modules** -- no React, Vue, or Angular. The frontend is a single-page application (SPA) built with:
...
- **Recent colors** -- Last 12 used colors stored in localStorage (`odysseus-recent-colors`)
...
- **Favorites** -- Star models for quick access, persisted in localStorage (`odysseus-model-favorites`)
- **Usage tracking** -- Records selection count and last-used time (`odysseus-model-usage`)
...
**Settings persistence**: Settings saved to localStorage (`odysseus-research-settings`) and restored on panel open.
