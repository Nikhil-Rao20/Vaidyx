...
**odysseus-mlx-image-bridge**.  It produces two standalone command-line
executables that run Apple MLX machine-learning models for image processing on
Apple Silicon Macs:

| Executable | Purpose | Swift Target |
|---|---|---|
| `odysseus-mlx-colorize` | Automatic grayscale-to-color image colorization | `OdysseusMLXColorize` |
| `odysseus-mlx-inpaint` | Object removal / image inpainting | `OdysseusMLXInpaint` |
...
interface that the Odysseus Python backend can call via `subprocess`.
...
**File:** `swift/odysseus-mlx-image-bridge/Package.swift` (lines 1-30)
...
|---|---|---|
| `OdysseusMLXInpaint` | `.executableTarget` | `LaMa`, `MIGAN` (from mlx-lama-swift) |
| `OdysseusMLXColorize` | `.executableTarget` | `DDColor` (from mlx-ddcolor-swift) |
...
## 3. Colorize Module -- OdysseusMLXColorize

**File:** `swift/odysseus-mlx-image-bridge/Sources/OdysseusMLXColorize/main.swift` (lines 1-80)
...
```
odysseus-mlx-colorize --model weights.safetensors --image input.png --output output.png [--tier tiny|large]
```
...
## 4. Inpaint Module -- OdysseusMLXInpaint

**File:** `swift/odysseus-mlx-image-bridge/Sources/OdysseusMLXInpaint/main.swift` (lines 1-88)
...
```
odysseus-mlx-inpaint --model weights.safetensors --image input.png --mask mask.png --output output.png [--mode best|fast]
```
...
**File:** `swift/odysseus-mlx-image-bridge/Package.swift`
...
```bash
cd swift/odysseus-mlx-image-bridge
swift build -c release
```

The resulting binaries are placed at:
```
.build/release/odysseus-mlx-colorize
.build/release/odysseus-mlx-inpaint
```
...
Odysseus dependency-management system (in `routes/shell_routes.py` and
...
Browser/Client
    |
    v
Odysseus Python Backend (app.py / routes)
    |
    v
scripts/mlx_image_server.py  (FastAPI, port 8100)
    |
    v  subprocess.run(...)
odysseus-mlx-colorize  or  odysseus-mlx-inpaint
    |
...
|---|---|
| `odysseus-mlx-colorize` | `mlx-ddcolor-serve` |
| `odysseus-mlx-inpaint` | `mlx-lama-serve` |
...
The Odysseus Cookbook UI checks whether the Swift bridge binaries are installed
...
(
    ...
    "LaMa / MI-GAN MLX inpainting requires an Odysseus-compatible mlx-lama-swift bridge ...",
