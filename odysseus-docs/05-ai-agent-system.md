...
> Comprehensive documentation of the Odysseus AI/Agent system architecture, covering the full stack from user message to AI response, including tool calling, model management, search integration, and multi-model support.
...
Odysseus is a self-hosted AI assistant built as a FastAPI application with a modular agent architecture. The system supports both simple chat interactions and autonomous agent loops with tool calling. Its key architectural layers are:
