...
1. **Composer guard** (L6-33) -- Marks `window.__odysseusComposerUserEdited` on
...
2. **User-switch wipe** (L43-91) -- Fetches `/api/auth/status`; if the logged-in
   user differs from the cached `odysseus-auth-user` key, wipes all localStorage
...
- **Default chat cache** (L207-227) -- Fetches `/api/default-chat` at load,
  caches in localStorage as `odysseus-default-chat-cache`.
...
- Folder groups with expand/collapse state persisted to
  `odysseus-folder-state` / `odysseus-folder-order`
...
|-----|-------------------|---------|
| THEME | `odysseus-theme` | Active theme name |
| TOGGLES | `odysseus-toggles` | Tool toggle states (JSON object) |
...
| MODEL_SELECTED | `odysseus-selected-model` | Last selected model |
| MODEL_ENDPOINTS | `odysseus-model-endpoints` | Cached endpoint list |
| SORT_ORDER | `odysseus-sessions-sort` | Session sort mode |
| INCOGNITO | `odysseus-incognito` | Incognito mode state |
| RAG_ACTIVE | `odysseus-rag-active` | RAG toggle |
| MCP_ACTIVE | `odysseus-mcp-active` | MCP toggle |
| DENSITY | `odysseus-density` | UI density preference |
| UI_SCALE | `odysseus-ui-scale` | UI scale factor |
| WORKSPACE | `odysseus-workspace` | Active workspace |
...
- `getToggle(name, fallback)` / `setToggle(name, value)` -- Per-toggle
  read/write within the `odysseus-toggles` JSON object
...
Cache name: `odysseus-v376-settings-title-icons` (bumped on logic/precache changes).
