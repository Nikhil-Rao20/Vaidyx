...
This document covers every deployment method, container configuration, GPU passthrough setup, platform-specific launcher, systemd service, networking, environment variables, and licensing for the Odysseus codebase.

---

## 1. Deployment Overview

Odysseus supports five distinct deployment methods:
...
| Native Linux (systemd) | Linux | `install-service.sh` + `odysseus-ui.service` | Direct hardware access |
...
1. **Creates user/group** (lines 22-27): Creates an `odysseus` group at `PGID` and user at `PUID` (both default to `1000`) unless they already exist
2. **Docker socket plumbing** (lines 36-48): When `ODYSSEUS_ENABLE_HOST_DOCKER=true` and `/var/run/docker.sock` is present, adds the app user to the Docker socket's group for host Docker access
...
- Docker passthrough test using `alpine:3.20` (configurable via `ODYSSEUS_AMD_TEST_IMAGE`)
...
2. **Port configuration** (lines 35-36):
   ...
   - Overrides: `ODYSSEUS_PORT` > `APP_PORT` > `7860`
   - Bind: `ODYSSEUS_HOST` > `APP_BIND` > `127.0.0.1`
...
11. **Browser auto-open** (lines 234-252): Background poller waits up to 90 seconds for the server, then opens the URL. Suppressed with `ODYSSEUS_NO_OPEN=1`
...
Produces:
- `dist/Odysseus.app` -- double-click launcher
...
dist/Odysseus.app/
  Contents/
    Info.plist          -- Bundle identifier: com.odysseus.launcher, min macOS 11.0
    MacOS/Odysseus      -- Bash launcher script
    Resources/
      odysseus.icns     -- Icon (center-cropped from docs/odysseus.jpg)
```
...
- Default port: `${ODYSSEUS_PORT:-7860}`
...
```
dist\Odysseus\
  Odysseus.exe            -- PyInstaller-frozen launcher
...
3. Runs PyInstaller with `--onedir --noconsole --icon=static/icon.ico --name Odysseus`
...
- **System tray icon** (lines 69-109): Creates a pystray icon with "Open Odysseus" and "Exit" menu items, using a programmatically generated sailing boat icon in brand colors
...
**File:** `/odysseus-ui.service` (17 lines)
...
**Important:** The slim Odysseus image does not bundle CUDA userspace or inference engines. After enabling GPU passthrough, install vLLM / llama-cpp-python / SGLang via Cookbook (Dependencies tab) or pip inside the container.
...
1. Checks if `/etc/searxng/settings.yml` is empty, contains the template placeholder `__SEARXNG_SECRET__`, or an old marker string `odysseus-local-searxng-json-2026-05-30`
...
- JSON format enabled -- required for Odysseus to query SearXNG programmatically
- HTML format enabled -- allows direct browser access to the SearXNG UI at `http://localhost:8080`

The `odysseus` service connects to SearXNG at `http://searxng:8080` (Docker internal network).
...
| `llmfit-MIT-LICENSE.txt` | MIT | llmfit (Alex Jones, 2026) | `services/hwfit/`, `routes/cookbook_*.py`, `routes/hwfit_routes.py`, `static/js/cookbook*.js`, `scripts/odysseus-cookbook` |
...
| `ODYSSEUS_ADMIN_PASSWORD` | (empty) | Admin password (generated on first run if empty) |
...
| Variable | Default | Description |
|---|---|---|
| `ODYSSEUS_CHAT_UPLOAD_MAX_BYTES` | `10485760` (10 MB) | Chat file upload limit |
| `ODYSSEUS_GALLERY_UPLOAD_MAX_BYTES` | `104857600` (100 MB) | Gallery upload limit |
| `ODYSSEUS_GALLERY_TRANSFORM_UPLOAD_MAX_BYTES` | `26214400` (25 MB) | Gallery transform upload limit |
| `ODYSSEUS_MEMORY_IMPORT_MAX_BYTES` | `10485760` (10 MB) | Memory import limit |
| `ODYSSEUS_PERSONAL_UPLOAD_MAX_BYTES` | `26214400` (25 MB) | Personal file upload limit |
| `ODYSSEUS_EMAIL_COMPOSE_UPLOAD_MAX_BYTES` | `26214400` (25 MB) | Email attachment limit |
| `ODYSSEUS_STT_MAX_AUDIO_BYTES` | `26214400` (25 MB) | Speech-to-text audio limit |
| `ODYSSEUS_ICS_MAX_BYTES` | `10485760` (10 MB) | Calendar ICS import limit |
| `ODYSSEUS_TTS_CACHE_MAX_BYTES` | (unset) | TTS cache size limit |
...
| `ODYSSEUS_INPROCESS_POLLERS` | `1` | Number of in-process pollers |
| `ODYSSEUS_INPROCESS_TASKS` | `1` | Number of in-process tasks |
| `ODYSSEUS_SCRIPT_HOST` | `localhost` | Script execution host |
...
| `ODYSSEUS_ENABLE_HOST_DOCKER` | (unset) | Enable host Docker socket access |
...
| Variable | Default | Description |
|---|---|---|
| `ODYSSEUS_PORT` | `7860` | macOS app port (overrides `APP_PORT`) |
| `ODYSSEUS_HOST` | `127.0.0.1` | macOS bind host (overrides `APP_BIND`) |
| `ODYSSEUS_NO_OPEN` | (unset) | Suppress auto-opening browser |
| `ODYSSEUS_SKIP_RUN_HINT` | (unset) | Suppress setup.py run hint |
...
|---|---|---|---|
| Odysseus (app) | 7000 | `${APP_BIND:-127.0.0.1}:${APP_PORT:-7000}` | `APP_BIND`, `APP_PORT` |
...
- Odysseus reaches SearXNG at `http://searxng:8080`
- Odysseus reaches ChromaDB at `chromadb:8000`
...
The `odysseus` container will not start until:
...
**Bind-mounted volumes (Odysseus container):**
...
```powershell
powershell -ExecutionPolicy Bypass -File .\build-windows-portable.ps1
# Run: dist\Odysseus\Odysseus.exe
```

### Linux systemd
```bash
# Edit odysseus-ui.service with your username and paths
./install-service.sh
