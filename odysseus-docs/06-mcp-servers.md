...
Both the email and memory servers implement **owner-scoped access**. Environment variables (`ODYSSEUS_MCP_EMAIL_OWNER`, `ODYSSEUS_MCP_MEMORY_OWNER`) or a `_odysseus_owner` argument injected by the agent identify the authenticated user. When owner-scoped rows exist in the database, the server filters results to only those belonging to the current owner, preventing cross-user data leaks.
...
- **Description:** List the email accounts configured in Odysseus. Returns each account's name, email address, and whether it is the default.
...
- **Description:** Create a new Odysseus email compose draft document. Does NOT send. Creates a reviewable Document record with `language="email"` containing To/Cc/Bcc/Subject headers and body separated by `---`. Respects the user's configured writing style from Settings.
- **Parameters:**
  ...
  - `cc` (string, optional) -- CC addresses.
  - `bcc` (string, optional) -- BCC addresses.
  - `title` (string, optional) -- Odysseus document title.
  - `account` (string, optional) -- Which email account.
...
- **Description:** Create an Odysseus email reply draft document for an existing email UID. Does NOT send. Threads the draft with proper In-Reply-To/References and prefills recipient/subject.
- **Parameters:**
  - `uid` (string, required) -- Email UID.
...
- **Description:** Generate an AI reply using Odysseus' AI reply prompt and writing style, then create a compose draft document. Does NOT send.
- **Parameters:**
  - `uid` (string, required) -- Email UID.
  - `folder` (string, default `"INBOX"`) -- IMAP folder.
  - `reply_all` (boolean, default `false`) -- Reply to all.
...
- `--isolated` unless `ODYSSEUS_BROWSER_ISOLATED=0`.
- `--no-sandbox` unless `ODYSSEUS_BROWSER_NO_SANDBOX=0`.

Cache directory: `data/local/playwright-mcp-cache/` (configurable via `ODYSSEUS_BROWSER_MCP_CACHE`).
...
|----------|---------|-------------|
| `ODYSSEUS_DISABLE_MCP` | `""` | Set to `1`/`true`/`yes` to disable all built-in MCP servers |
| `ODYSSEUS_BROWSER_MCP_REQUIRE_CACHE` | `""` | Set to `1`/`true` to require the Playwright package to be pre-cached (no auto-install on startup) |
| `ODYSSEUS_BROWSER_MCP_CACHE` | `data/local/playwright-mcp-cache` | Cache directory for the browser MCP server |
| `ODYSSEUS_BROWSER_EXECUTABLE` | auto-detected | Path to Chrome/Chromium binary for browser MCP |
| `ODYSSEUS_BROWSER_ISOLATED` | `"1"` | Run browser in isolated mode (no persistent profile) |
| `ODYSSEUS_BROWSER_NO_SANDBOX` | `"1"` | Run browser with `--no-sandbox` |
| `ODYSSEUS_STARTUP_WARMUPS` | `""` | Enable tool index pre-warming on startup |
...
| `ODYSSEUS_MCP_EMAIL_OWNER` | `""` | Owner identity for owner-scoped email access |
| `ODYSSEUS_EMAIL_OWNER` | `""` | Alternate owner identity variable |
| `ODYSSEUS_DOCUMENT_OWNER` | `""` | Override owner for MCP-created draft documents |
| `ODYSSEUS_MAIL_ATTACHMENTS_DIR` | `data/mail-attachments` | Directory for downloaded email attachments |

#### Memory Server

| Variable | Default | Description |
|----------|---------|-------------|
| `ODYSSEUS_MCP_MEMORY_OWNER` | `""` | Owner identity for owner-scoped memory access |
| `ODYSSEUS_MEMORY_OWNER` | `""` | Alternate owner identity variable |
