...
This document covers the Odysseus test suite, CLI/utility scripts, and architectural specifications. It is organized into ten sections matching the major structural areas.
...
Odysseus uses **pytest** as its Python testing framework and **Node.js `node:test`** for JavaScript/streaming tests. Configuration lives in `pyproject.toml`:
...
The 734 test files in `tests/` (top-level) cover every major Odysseus feature. The largest files by test count and line count:
...
```python
from tests.helpers.cli_loader import load_script
cli = load_script("odysseus-webhook")  # loads scripts/odysseus-webhook
```
...
- **20 `odysseus-*` CLI tools** -- Unix-style subcommand CLIs for every feature
- **1 `odysseus` dispatcher** -- umbrella command that discovers and dispatches to sub-CLIs
...
All `odysseus-*` CLIs share a common pattern via `scripts/_lib/cli.py` (122 lines):
...
### The `odysseus` Dispatcher (`scripts/odysseus`, 134 lines)

Entry point for all CLI tools. Discovers `scripts/odysseus-<name>` siblings and dispatches to them (like `git` finds `git-foo`):

```bash
odysseus                    # list all subcommands
odysseus mail list --pretty # runs odysseus-mail list --pretty
odysseus help mail          # shows odysseus-mail --help
odysseus --version          # prints version
```

Runs subcommands with the project's venv Python so dependencies resolve. Can be symlinked to `~/.local/bin/odysseus`.

---

## 7. Shell Completion

### Bash Completion (`scripts/_completion/odysseus.bash`, 92 lines)
...
Features:
- First word after `odysseus`: completes with subcommand names
- Subsequent words: completes with subcommand-specific subcommands (cached by parsing `--help` output)
- Works for both `odysseus mail <tab>` and direct `odysseus-mail <tab>`
- Lazy cache refresh via `_odysseus_refresh_cache`

### Zsh Completion (`scripts/_completion/odysseus.zsh`, 72 lines)
...
Creates/removes a switchable "Demo" EmailAccount in the Odysseus database. Points at a local Dovecot instance (`localhost:31143`, STARTTLS) with password stored via Fernet encryption.
...
All files follow the `load_script` + `make_core_db_stub` pattern and exercise internal `_` functions of the corresponding `scripts/odysseus-*` CLI.
...
| File | Lines | Category |
|---|---:|---|
| `odysseus` | 134 | Dispatcher |
| `odysseus-backup` | 272 | CLI: backup |
| `odysseus-calendar` | 259 | CLI: calendar |
| `odysseus-contacts` | 147 | CLI: contacts |
| `odysseus-cookbook` | 555 | CLI: cookbook |
| `odysseus-docs` | 203 | CLI: documents |
| `odysseus-gallery` | 185 | CLI: gallery |
| `odysseus-logs` | 147 | CLI: logs |
| `odysseus-mail` | 406 | CLI: email |
| `odysseus-mcp` | 206 | CLI: MCP servers |
| `odysseus-memory` | 157 | CLI: AI memory |
| `odysseus-notes` | 176 | CLI: notes |
| `odysseus-personal` | 127 | CLI: personal docs |
| `odysseus-preset` | 139 | CLI: presets |
| `odysseus-research` | 189 | CLI: research |
| `odysseus-sessions` | 153 | CLI: sessions |
| `odysseus-signature` | 142 | CLI: signatures |
| `odysseus-skills` | 163 | CLI: skills |
| `odysseus-tasks` | 163 | CLI: tasks |
| `odysseus-theme` | 199 | CLI: themes |
| `odysseus-webhook` | 156 | CLI: webhooks |
| `_lib/cli.py` | 122 | Shared CLI scaffolding |
| `_lib/__init__.py` | 0 | Package marker |
| `_completion/odysseus.bash` | 92 | Bash tab-completion |
| `_completion/odysseus.zsh` | 72 | Zsh tab-completion |
