...
| POST   | `/api/email/compose-from-odysseus`          | Attach Odysseus doc/gallery item    |
| POST   | `/api/email/compose-from-odysseus-zip`      | Attach multiple items as ZIP        |
...
- **LocalStorage keys**: `odysseus-doc-open-{sid}` and `odysseus-doc-minimized-{sid}` track panel state per session
- **Email local drafts**: Reply drafts saved to localStorage under `odysseus.email.replyDraft.v1:` prefix, restored on reopen
