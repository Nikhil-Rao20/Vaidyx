# Odysseus Core Architecture

> Comprehensive reference for the core infrastructure of the Odysseus application.
...
Odysseus has two entry points: `app.py` (the primary server orchestrator) and `launcher.py` (a Windows-specific GUI wrapper).
...
- **Splash screen** (lines 35-66): When running from a frozen PyInstaller bundle (`sys.frozen == True`), immediately shows a tkinter splash window on a daemon thread -- dark-themed (`#1a1c23` background, `#e06c75` accent), centered, 360x160 pixels, displays "Odysseus -- Launching background services...".

- **System tray icon** (lines 69-109): Uses `pystray` and `Pillow` to create a 64x64 RGBA sailing-boat icon in the Odysseus brand red (`#e06c75`). Tray menu offers "Open Odysseus" (default action, opens browser) and "Exit" (calls `os._exit(0)`).
...
An interactive setup script that prepares the Odysseus environment from scratch. Safe to re-run (skips what already exists).
...
6. **`create_default_admin()`** (lines 89-153): Creates the initial admin user in `auth.json`. Credential priority: environment variables (`ODYSSEUS_ADMIN_USER`, `ODYSSEUS_ADMIN_PASSWORD`) > interactive prompt > auto-generated random password. Uses `bcrypt.hashpw` for password hashing. Validates against `RESERVED_USERNAMES`. Minimum password length enforced via `PASSWORD_MIN_LENGTH` (8 characters).
...
3. **Internal tool token**: `X-Odysseus-Internal-Token` header + trusted loopback = admin access. Supports `X-Odysseus-Owner` header for user impersonation.
...
**`class _SlowRequestLogMiddleware`**: Logs a warning for any request that takes longer than the threshold (default 0.75s, configurable via `ODYSSEUS_SLOW_REQUEST_LOG_SECONDS`). Includes method, path, status, and elapsed time.
...
- **Allowed headers**: Accept, Authorization, Content-Type, X-API-Key, X-Auth-Token, X-Odysseus-Internal-Token, X-Odysseus-Owner, X-Requested-With, X-TZ-Offset
- **Credentials**: Allowed
...
| `DATA_DIR` | `ODYSSEUS_DATA_DIR` env or `get_default_data_dir()` | Writable data directory |
...
|---|---|---|
| `ODYSSEUS_DATA_DIR` | Platform-dependent | Writable data directory |
...
| `ODYSSEUS_SLOW_REQUEST_LOG_SECONDS` | `"0.75"` | Slow request logging threshold |
| `ODYSSEUS_INTERNAL_BASE` | Auto-derived | Base URL for internal loopback calls |
| `ODYSSEUS_INTERNAL_TOKEN` | Auto-generated | Internal tool auth token |
| `ODYSSEUS_STARTUP_WARMUPS` | `""` (disabled) | Enable startup warmups |
| `ODYSSEUS_MODEL_KEEPALIVE` | `""` (disabled) | Enable periodic endpoint keepalive |
| `ODYSSEUS_INPROCESS_TASKS` | `"1"` (enabled) | Enable in-process task scheduler |
| `ODYSSEUS_ADMIN_USER` | `""` | Setup: admin username |
| `ODYSSEUS_ADMIN_PASSWORD` | `""` | Setup: admin password |
| `ODYSSEUS_SKIP_ADMIN_PROMPT` | Not set | Setup: skip interactive prompt |
| `ODYSSEUS_SKIP_RUN_HINT` | Not set | Setup: suppress "start the server" hint |
...
| `ODYSSEUS_MAIL_ATTACHMENTS_DIR` | `{DATA_DIR}/mail-attachments` | Email attachments directory |
