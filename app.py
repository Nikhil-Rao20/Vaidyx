...
try:
    ...
    # Vaidyx is single-process by convention, so this is acceptable, but be aware that
...
app = FastAPI(
    title="Vaidyx",
    description="Healthcare AI platform for medical professionals",
    version="1.0.0",
)
...
class _SlowRequestLogMiddleware(_BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        try:
            ...
            try:
                threshold = float(os.getenv("VAIDYX_SLOW_REQUEST_LOG_SECONDS", "0.75") or "0.75")
            except Exception:
                threshold = 0.75
...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. Vaidyx's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-Vaidyx-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-Vaidyx-Owner") or "").strip()
...
app.include_router(setup_backup_routes(memory_manager, skills_manager))
...
async def _startup_event():
    ...
    _startup_warmups_enabled = str(os.getenv("VAIDYX_STARTUP_WARMUPS", "")).lower() in {"1", "true", "yes", "on"}
    if _startup_warmups_enabled:
        ...
        logger.info("Startup warmups disabled (set VAIDYX_STARTUP_WARMUPS=1 to enable)")
    ...
    _keepalive_enabled = str(os.getenv("VAIDYX_MODEL_KEEPALIVE", "")).lower() in {"1", "true", "yes", "on"}
    ...
    # `VAIDYX_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("VAIDYX_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (VAIDYX_INPROCESS_TASKS=0); "
    ...
    _startup_tasks.append(asyncio.create_task(_auto_detect_ollama()))

    logger.info("Application startup complete")


async def _auto_detect_ollama():
    """Probe localhost:11434 for Ollama; create a ModelEndpoint if not already registered."""
    import httpx
    ...
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            ...
            if not r.is_success:
                logger.debug("Ollama not detected at %s", OLLAMA_URL)
                return
    except Exception:
    ...
    try:
        existing = (
            ...
            .filter(ModelEndpoint.base_url.in_([OLLAMA_V1, OLLAMA_URL, f"{OLLAMA_URL}/"]))
            .first()
        )
        if existing:
            logger.info("Ollama endpoint already registered (id=%s)", existing.id)
            return
        ...
        ep = ModelEndpoint(
            ...
            api_key=None,
            is_enabled=True,
            cached_models=_json.dumps(models) if models else None,
            endpoint_kind="local",
            model_refresh_mode="auto",
        )
        db.add(ep)
        db.commit()
        logger.info("Auto-registered Ollama endpoint with %d model(s)", len(models))
    except Exception as e:
        logger.warning("Ollama auto-detect failed: %s", e)
        db.rollback()
    finally:
        db.close()
