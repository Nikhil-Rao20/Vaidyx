...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When Vaidyx runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends Vaidyx' own interpreter directory to PATH. If Vaidyx itself is
...
def _append_pip_install_runner_lines(runner_lines: list[str], cmd: str) -> None:
    ...
    runner_lines.append('  echo "[vaidyx] pip does not support --break-system-packages; installing without it."')
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by Vaidyx.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$VAIDYX_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $VAIDYX_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$VAIDYX_PREFLIGHT_EXIT"')
    runner_lines.append('fi')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for Vaidyx-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('VAIDYX_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$VAIDYX_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  VAIDYX_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[vaidyx] vLLM CLI: $VAIDYX_VLLM_BIN"')
    runner_lines.append('  VAIDYX_VLLM_VERSION="$("$VAIDYX_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$VAIDYX_VLLM_VERSION" ]; then echo "[vaidyx] vLLM version: $VAIDYX_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('VAIDYX_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $VAIDYX_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $VAIDYX_CMD_EXIT ==="')
        ...
        runner_lines.append('echo ""; echo "=== Process exited with code $VAIDYX_CMD_EXIT ==="')
        runner_lines.append('exit "$VAIDYX_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    # every OS-package dep from the launch path. Sets _vaidyx_have_prebuilt=1
    ...
    runner_lines.append('    _vaidyx_have_prebuilt=""')
    runner_lines.append('    _vaidyx_arch="$(uname -m)"')
    runner_lines.append('    _vaidyx_prebuilt_url=""')
    runner_lines.append('    if command -v curl >/dev/null 2>&1 && [ "$_vaidyx_arch" = "x86_64" ]; then')
    runner_lines.append('      _vaidyx_pat=""')
    runner_lines.append('      _vaidyx_has_nv_inline() { command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L 2>/dev/null | grep -q "GPU "; }')
    runner_lines.append('      _vaidyx_has_vk_inline() { ldconfig -p 2>/dev/null | grep -q "libvulkan\\.so" || command -v vulkaninfo >/dev/null 2>&1 || [ -e /usr/lib/x86_64-linux-gnu/libvulkan.so.1 ]; }')
    runner_lines.append('      _vaidyx_has_vkdev_inline() { ls /dev/dri/renderD* >/dev/null 2>&1 || (lspci 2>/dev/null | grep -Ei \'VGA|3D|Display\' | grep -Eiq \'AMD|ATI|Radeon\'); }')
    runner_lines.append('      if _vaidyx_has_nv_inline; then')
    runner_lines.append('        _vaidyx_pat="ubuntu.*cuda"')
    runner_lines.append('      elif _vaidyx_has_vkdev_inline && _vaidyx_has_vk_inline; then')
    runner_lines.append('        _vaidyx_pat="ubuntu.*vulkan"')
    runner_lines.append('      else')
    runner_lines.append('        _vaidyx_pat="ubuntu-x64\\\\.zip"')
    runner_lines.append('      fi')
    runner_lines.append('      _vaidyx_prebuilt_url="$(curl -fsSL --max-time 15 https://api.github.com/repos/ggml-org/llama.cpp/releases/latest 2>/dev/null | grep \'"browser_download_url"\' | cut -d\'"\' -f4 | grep -iE "$_vaidyx_pat" | grep -iv "arm\\|aarch64" | head -1)"')
    runner_lines.append('    fi')
    ...
    runner_lines.append('    if [ -n "$_vaidyx_prebuilt_url" ] && (command -v unzip >/dev/null 2>&1 || command -v bsdtar >/dev/null 2>&1 || command -v python3 >/dev/null 2>&1); then')
    runner_lines.append('      echo "[vaidyx] Found prebuilt llama-server: $_vaidyx_prebuilt_url"')
    runner_lines.append('      mkdir -p ~/bin "$HOME/.cache/vaidyx/llama-cpp-prebuilt" && cd "$HOME/.cache/vaidyx/llama-cpp-prebuilt"')
    runner_lines.append('      rm -f llama-cpp.zip')
    runner_lines.append('      if curl -fsSL --max-time 120 "$_vaidyx_prebuilt_url" -o llama-cpp.zip && [ -s llama-cpp.zip ]; then')
    ...
    runner_lines.append('        _vaidyx_extracted="$(find build -type f -name llama-server 2>/dev/null | head -1)"')
    runner_lines.append('        if [ -n "$_vaidyx_extracted" ]; then')
    runner_lines.append('          chmod +x "$_vaidyx_extracted"')
    runner_lines.append('          ln -sf "$_vaidyx_extracted" ~/bin/llama-server')
    runner_lines.append('          _vaidyx_libdir="$(dirname "$_vaidyx_extracted")"')
    runner_lines.append('          mkdir -p ~/.config && echo "export LD_LIBRARY_PATH=\\"$_vaidyx_libdir:\\${LD_LIBRARY_PATH:-}\\"" > ~/.config/vaidyx-llama-cpp-env')
    runner_lines.append('          _vaidyx_have_prebuilt=1')
    runner_lines.append('          echo "[vaidyx] Prebuilt llama-server installed at $_vaidyx_extracted"')
    runner_lines.append('        fi')
    runner_lines.append('      fi')
    runner_lines.append('      [ -z "$_vaidyx_have_prebuilt" ] && echo "[vaidyx] Prebuilt download/extract failed — falling back to from-source build."')
    runner_lines.append('    elif [ -z "$_vaidyx_prebuilt_url" ]; then')
    runner_lines.append('      echo "[vaidyx] No matching prebuilt llama-server for this host (arch=$_vaidyx_arch) — will build from source."')
    runner_lines.append('    fi')
    runner_lines.append('  if [ -z "$_vaidyx_have_prebuilt" ]; then')
    ...
    runner_lines.append('    _vaidyx_has_nvidia_hw() {')
    ...
    runner_lines.append('    }')
    runner_lines.append('    if _vaidyx_has_nvidia_hw; then')
    ...
    runner_lines.append('    _vaidyx_apt_bootstrap() {')
    ...
    runner_lines.append('        echo "[vaidyx] Auto-installing missing build deps via apt:$_missing"')
    ...
    runner_lines.append('        echo "[vaidyx] Auto-installing missing build deps via pacman:$_missing"')
    ...
    runner_lines.append('        echo "[vaidyx] Auto-installing missing build deps via dnf:$_missing"')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[vaidyx] WARNING: missing build deps ($_missing) — passwordless sudo is unavailable, cannot auto-install. Cookbook Diagnosis will explain the fix after the build fails."')
    runner_lines.append('      fi')
    runner_lines.append('    }')
    runner_lines.append('    _vaidyx_apt_bootstrap')
    runner_lines.append('    _vaidyx_missing_build_deps=""')
    runner_lines.append('    command -v cmake >/dev/null 2>&1 || _vaidyx_missing_build_deps="$_vaidyx_missing_build_deps cmake"')
    runner_lines.append('    command -v git >/dev/null 2>&1 || _vaidyx_missing_build_deps="$_vaidyx_missing_build_deps git"')
    runner_lines.append('    command -v g++ >/dev/null 2>&1 || command -v gcc >/dev/null 2>&1 || _vaidyx_missing_build_deps="$_vaidyx_missing_build_deps build-essential"')
    runner_lines.append('    if [ -n "$_vaidyx_missing_build_deps" ]; then')
    runner_lines.append('      echo "ERROR: llama.cpp source build needs missing packages:$_vaidyx_missing_build_deps"')
    ...
    runner_lines.append('    _vaidyx_has_vulkan() {')
    ...
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    runner_lines.append('    _vaidyx_has_vulkan_device() {')
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[vaidyx] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('    elif command -v nvcc &>/dev/null && _vaidyx_has_nvidia_hw; then')
    runner_lines.append('      rm -rf build')
    ...
    runner_lines.append('      _vaidyx_has_cudart() {')
    ...
    runner_lines.append('        return 1')
    runner_lines.append('      }')
    runner_lines.append('      if _vaidyx_has_cudart; then')
    runner_lines.append('        echo "[vaidyx] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[vaidyx] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[vaidyx]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[vaidyx]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    elif _vaidyx_has_vulkan_device && _vaidyx_has_vulkan; then')
    runner_lines.append('      echo "[vaidyx] Vulkan-capable GPU detected (no ROCm/CUDA toolchain installed) — building llama-server with Vulkan support..."')
    ...
    runner_lines.append('    else')
    runner_lines.append('      echo "[vaidyx] WARNING: no HIP/CUDA/Vulkan toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[vaidyx]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[vaidyx]   Install Vulkan (libvulkan-dev) / ROCm for AMD GPUs or CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      rm -rf build')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
    runner_lines.append('  fi  # end _vaidyx_have_prebuilt guard')
...
def _llama_cpp_rebuild_cmd(update_source: bool = False) -> str:
    ...
    if update_source:
        update_cmd = (
            ...
            'echo "[vaidyx] WARNING: llama.cpp source update failed; clearing cached build anyway."; '
            'elif command -v git >/dev/null 2>&1; then '
            'git clone --depth 1 https://github.com/ggml-org/llama.cpp "$HOME/llama.cpp" || '
            'echo "[vaidyx] WARNING: llama.cpp clone failed; clearing cached build anyway."; '
            'fi && '
        )
    return (
        'mkdir -p "$HOME/bin" && '
        ...
        'echo "[vaidyx] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\vaidyx-sessions"
...
def _diagnose_serve_output(text: str) -> dict | None:
    ...
    patterns = [
        ...
        (
            ...
            "LaMa / MI-GAN MLX inpainting requires an Vaidyx-compatible mlx-lama-swift bridge on this Apple Silicon server.",
            [{"label": "build mlx-lama-swift bridge and put vaidyx-mlx-inpaint or mlx-lama-serve on PATH", "op": "dependency", "package": "mlx_lama_swift"}],
        ),
        (
            r"mlx-ddcolor-swift|vaidyx-mlx-colorize|mlx-ddcolor-serve|DDColor MLX models require",
            "DDColor MLX colorization requires an Vaidyx-compatible mlx-ddcolor-swift bridge on this Apple Silicon server.",
            [{"label": "build mlx-ddcolor-swift bridge and put vaidyx-mlx-colorize or mlx-ddcolor-serve on PATH", "op": "dependency", "package": "mlx_ddcolor_swift"}],
        ),
        (
