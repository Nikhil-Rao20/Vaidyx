...
FUNCTION_TOOL_SCHEMAS = [
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "name": {"type": "string", "description": "For toggle: web, bash, research, incognito, document_editor (aliases: shell, search, deepresearch, documents). For open_panel: documents, sessions, notes, brain/memories, skills, settings, cookbook. For set_theme: a preset theme name. For create_theme: the custom theme name."},
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Manage calendar events: list events in a date range, create, update, delete. Each event can carry a tag/category (event_type) and importance level. Resolve relative dates like today/tomorrow against the 'Current date and time' system context, then pass ISO 8601 datetimes in the user's local wall time; for all-day events set all_day=true and pass YYYY-MM-DD. For event reminders/alarms, pass reminder_minutes; the tool creates the Vaidyx note reminder, so do not also call manage_notes for the same reminder. Do not set rrule for single-occurrence requests such as 'next Wednesday only'; use rrule only when the user explicitly wants recurrence.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string",
                    ...
                    "reminder_minutes": {"type": "integer", "description": "For create_event: create an Vaidyx reminder this many minutes before the event, e.g. 5 for 'reminder 5 min before'."},
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Manage user preferences and settings. Use `disable_tool`/`enable_tool`/`list_tools` to turn individual tools on or off globally (e.g. shell, search, browser, documents, memory, skills, tasks, notes, calendar). Use list/get/set/delete for free-form preferences.",
            "parameters": {
                "type": "object",
                "properties": {
                    ...
                    "tool": {"type": "string", "description": "Tool name to disable/enable (for disable_tool/enable_tool). Accepts aliases: shell, search, browser, documents, memory, skills, tasks, notes, calendar — or a raw tool name like 'web_search'."}
                },
                "required": ["action"]
            }
        }
...
def function_call_to_tool_block(name: str, arguments: str) -> Optional[ToolBlock]:
    ...
    # the legacy empty-object coercion for stream robustness.
    ...
    if tool_type == "web_search":
        ...
        if action == "toggle":
            ...
            content = action
                        ...
                        "manage_endpoints", "manage_documents", "manage_settings"):
        content = json.dumps(args)
