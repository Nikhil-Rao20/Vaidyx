...
Streaming agent loop for vaidyx-ui.
...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- Vaidyx chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
_DOMAIN_TOOL_MAP = {
    ...
    "files": {"read_file", "write_file", "edit_file", "apply_patch", "todowrite", "grep", "glob", "ls", "get_workspace", "manage_bg_jobs"},
    "settings": {"manage_settings", "manage_endpoints", "app_api"},
...
Notes, checklists, AND user reminders. Use this for "create/add/write a note", todos, checklists, and "remind me to X at <time>" — never use memory for note content. For reminders, pair a short `title` (what to do) with a `due_date` (when). `due_date` accepts natural language ("tomorrow at 1pm", "in 2 hours", "next monday 9am") or ISO ("2026-05-12T13:00:00"). Actions: `list`, `add` (title, content OR items:[{text,done}], note_type, color, label, due_date), `update`, `delete`, `toggle_item`.""",
    "resolve_contact": "- ```resolve_contact``` — Look up a contact by name. Searches CardDAV address book. Args (JSON): {\"name\": \"...\"}.",
    "manage_contact": "- ```manage_contact``` — Create/update/delete/list CardDAV contacts. Args (JSON): {\"action\": \"list|add|update|delete\", \"name\": \"...\", \"email\": \"...\", \"phones\": [...], \"address\": \"...\", \"uid\": \"...\"}. For update/delete, call action=list first for the uid.",
    "manage_calendar": """\
```manage_calendar
...
```
GENERIC LOOPBACK to allowed Vaidyx internal endpoints. Use this whenever the user wants something the UI can do but there's NO named tool for it. Many UI buttons hit /api/* endpoints — you can hit allowed ones. Auth is handled automatically.
...
Blocked paths/routes (refused for safety): /api/auth/, /api/users/, /api/tokens/, /api/admin/, /api/backup/restore, POST /api/cookbook/packages/install, POST /api/cookbook/rebuild-engine, POST /api/cookbook/kill-pid.""",
}
...
_MCP_KEYWORDS = frozenset(["browse", "browser", "website", "calendar", "event",
...
def _local_computer_rules() -> str:
    return (
        "\n\n## Vaidyx Terminus local-machine mode\n"
...
def _minimal_saved_memory_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    if not facts:
        return None
    logger.info("[agent-intent] vaidyx doc minimal memory facts=%s", len(facts))
    return {
        "role": "user",
        "content": (
            "Saved user memory facts from Vaidyx Brain. These are the same "
...
def _minimal_recent_notes_tool_context_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    return {
        "role": "user",
        "content": (
            "Recent Vaidyx tool context for follow-up references only. "
...
def _minimal_vaidyx_doc_messages(messages: List[Dict], active_document, stream_create: bool = False) -> List[Dict]:
    """Tiny prompt path for the Vaidyx document LoRA.
    ...
    if stream_create:
        system = (
            "You are Vaidyx. Create the requested document by streaming exactly one fenced block:\n"
            "```document\n"
            "Title\n"
            "markdown\n"
            "Document content\n"
        ...
        system = (
            "You are Vaidyx. Edit or suggest changes to the active document using exactly one fenced tool block when needed.\n"
            ...
            "After the tool succeeds, Vaidyx will answer Done."
        )
...
def _minimal_vaidyx_notes_messages(messages: List[Dict]) -> List[Dict]:
    """Tiny prompt path for Vaidyx notes/calendar/tasks LoRAs.

    The finetune is trained to emit Vaidyx notes/calendar/task tool calls
    without receiving the full tool schema or saved-context wrapper stack.
    """
    latest = _extract_last_user_message(messages)
    system = (
        "You are Vaidyx. Handle notes, reminders, calendar events, and scheduled tasks.\n"
...
def _minimal_vaidyx_general_messages(messages: List[Dict], include_memory: bool = False) -> List[Dict]:
    """Minimal fallback for Vaidyx finetunes outside domain-specific paths."""
    latest = _extract_last_user_message(messages)
    system = (
        "You are Vaidyx. Answer directly and briefly.\n"
        "Use Vaidyx tool-call format only when the user explicitly asks you to take an action.\n"
...
def _normalize_ody_qwen_text_artifacts(text: str) -> str:
    """Repair common dropped-final-letter artifacts from small Vaidyx LoRAs.

    This is intentionally scoped to the vaidyx-qwen3 runtime path. It is not
...
def _build_system_prompt(
    ...
    """Build agent system prompt, inject document context, merge consecutive system msgs."""
...
def _compute_final_metrics(
    ...
    # Context % should describe the prompt Vaidyx assembled, not provider
...
async def stream_agent_loop(
    ...
    _ody_qwen_finetune_model = (model or "").lower().startswith("vaidyx-qwen3")
    ...
    if not guide_only and _relevant_tools is not None:
        ...
        if (
            ...
            logger.info("[tool-rag] Workspace file/terminal request; using Vaidyx Terminus toolset")
    ...
    if _ody_doc_finetune_mode and _relevant_tools is not None:
        ...
        logger.info("[agent-intent] vaidyx doc finetune tool clamp=%s", sorted(_relevant_tools))
        ...
        logger.info("[agent-intent] vaidyx notes finetune tool clamp=%s", sorted(_relevant_tools))
        ...
        try:
            ...
            pass
        logger.info("[agent-intent] vaidyx general no-tool clamp active")
    ...
    if _ody_doc_finetune_mode and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_vaidyx_doc_messages(
            messages,
        ...
        mcp_schemas = []
        logger.info(
            "[agent-intent] vaidyx doc minimal prompt active active_doc=%s stream_create=%s messages=%s",
        ...
        messages = _minimal_vaidyx_notes_messages(messages)
        mcp_schemas = []
        logger.info(
            "[agent-intent] vaidyx notes minimal prompt active messages=%s",
            len(messages),
        )
    elif _ody_qwen_finetune_model and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_vaidyx_general_messages(
            messages,
            include_memory=True,
        )
        mcp_schemas = []
        logger.info(
            "[agent-intent] vaidyx general minimal prompt active include_memory=%s messages=%s",
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if _force_answer:
            ...
            # Vaidyx-Qwen fine-tunes are trained to emit Vaidyx tool calls
        ...
        if _ody_doc_stream_create_mode and tool_blocks:
            ...
            if create_idx is None:
                logger.info(
                    "[agent] vaidyx doc stream-create discarded non-create tool call(s): %s",
                ...
                if len(tool_blocks) > 1 or create_idx != 0:
                    logger.info(
                        "[agent] vaidyx doc stream-create keeping first create_document and dropping extras: %s",
                        [block.tool_type for block in tool_blocks],
                    )
        ...
        if _ody_qwen_finetune_model and tool_blocks:
            ...
            if _dropped_memory_lookup:
                logger.info(
                    "[agent-intent] vaidyx qwen dropped manage_memory lookup; answering from compact memory"
                )
        ...
        if _doc_stream_create_completed:
            ...
            logger.info("[agent] vaidyx doc stream-create completed after one create_document")
            break
        ...
        if _ody_doc_tool_completed:
            if not full_response.strip() or full_response.strip().startswith("```"):
                full_response = "Done."
                yield 'data: ' + json.dumps({"delta": "Done."}) + '\n\n'
            logger.info("[agent] vaidyx doc tool completed after one textual tool block")
            break

        if (_ody_notes_finetune_mode or _ody_qwen_finetune_model) and _ody_notes_tool_completed:
            logger.info("[agent] vaidyx completed from deterministic tool output")
            break
