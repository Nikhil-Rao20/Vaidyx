...
`window.__vaidyxActiveEmailAccount`. Every API call appends
...
(`vaidyx-email-last-seen-uid`). The dot color reflects urgency state from
...
| Method | Endpoint | Used In |
|--------|----------|---------|
| GET | `/api/email/vaidyx/reminders` | emailLibrary (line 2624) |
