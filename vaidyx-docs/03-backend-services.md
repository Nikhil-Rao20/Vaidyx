...
This document provides comprehensive documentation for the Vaidyx service layer located in `services/`. Every module, class, and significant function is documented with line references, external dependencies, configuration options, and integration details.
...
### Purpose
Detects system hardware (CPU, GPU, RAM), estimates model memory requirements, and ranks LLM/image models against the user's hardware to recommend what will run well. This is the engine behind the "Cookbook" feature in the Vaidyx UI.
...
- Size limit: 500MB default, configurable via `VAIDYX_TTS_CACHE_MAX_BYTES` env var
