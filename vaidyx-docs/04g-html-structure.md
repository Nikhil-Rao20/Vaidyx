# 04g - Vaidyx HTML Structure

Reference for the static HTML files that define the Vaidyx UI.
...
- Read `vaidyx-theme` from localStorage and apply all CSS custom properties (bg, fg, panel, border, red, advanced overrides, syntax highlighting colors) to `:root`.
...
#sidebar
  ...
  .sidebar-header
    ...
    .sidebar-brand                "Vaidyx" brand link
  .sidebar-inner
...
#chat-container .welcome-active
  h1.a11y-visually-hidden         "Vaidyx" (screen reader only)
  .chat-top-bar
...
body
  main.card
    h1.logo                      Boat SVG + "Vaidyx" wordmark
...
A standalone marketing/landing page for the Vaidyx project.

### Structure

```
nav (sticky)
  .brand                         Boat logo + "Vaidyx"
  .nav-links                     Features, Testimonials, How, Get started, GitHub

header.hero
  #hero-flow                     Perlin flow field canvas (particle streams)
  .hero-logo                     Large boat SVG + "Vaidyx" wordmark
...
Vaidyx uses **no icon font library**. All icons are inline SVGs embedded directly in the HTML. The boat logo is a custom SVG path used as:
