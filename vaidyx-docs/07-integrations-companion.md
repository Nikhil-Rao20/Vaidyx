...
This document covers the external agent integrations (Claude Code, Codex) and the companion LAN-pairing bridge that together form Vaidyx's external-access surface.

---

## 1. Integrations Overview

Vaidyx exposes a unified, scope-gated agent API under `/api/codex/*` that external AI coding agents use to read and write user data. Two integration bundles ship in the `integrations/` directory:

| Integration | Directory | Delivery | Runtime API |
|---|---|---|---|
| Claude Code | `integrations/claude/` | Skill bundle extracted to `~/.claude/skills/vaidyx/` | `/api/codex/*` (shared) |
| Codex | `integrations/codex/` | Plugin bundle extracted to `~/plugins/vaidyx/` | `/api/codex/*` (shared) |

Both integrations use the **same server-side routes** (`routes/codex_routes.py`). The `codex` path prefix is historic; all agent integrations share it. Authentication is via scoped `ody_`-prefixed Bearer tokens created through Vaidyx Settings > Integrations.
...
integrations/
  claude/
    ...
    skills/vaidyx/
      ...
      scripts/vaidyx_api.py              # CLI helper for calling scoped API endpoints
  codex/
    .codex-plugin/plugin.json              # Codex plugin manifest
    README.md                              # Setup instructions for Codex
    scripts/vaidyx_api.py                # CLI helper (identical logic to Claude version)
    skills/vaidyx/SKILL.md               # Skill definition Codex reads at runtime
```
...
|---|---|---|
| `VAIDYX_URL` | Base URL for the Vaidyx instance | `http://127.0.0.1:7000` |
| `VAIDYX_API_TOKEN` | Scoped API token (prefix `ody_`) | `ody_abc123...` |
...
1. User opens Vaidyx Settings > Integrations and adds a Claude Agent.
2. Vaidyx generates a scoped `ody_` token and shows setup commands.
3. User exports `VAIDYX_URL` and `VAIDYX_API_TOKEN` in their terminal.
4. User downloads and extracts the skill bundle:

```bash
curl -fsSL -H "Authorization: Bearer $VAIDYX_API_TOKEN" \
  "$VAIDYX_URL/api/claude/plugin.zip" -o /tmp/vaidyx-claude-skill.zip
python3 -m zipfile -e /tmp/vaidyx-claude-skill.zip ~/.claude/
```

Claude Code auto-loads anything under `~/.claude/skills/`, so the `vaidyx` skill becomes available in any session with the environment variables set.
...
### SKILL.md (`integrations/claude/skills/vaidyx/SKILL.md`)
...
---
name: vaidyx
description: Use when the user asks Claude Code to read or write Vaidyx data
  ...
  VAIDYX_URL and VAIDYX_API_TOKEN.
---
```
...
### vaidyx_api.py (`integrations/claude/skills/vaidyx/scripts/vaidyx_api.py`)
...
def _config() -> tuple[str, str] | None          # line 38 - Reads VAIDYX_URL and VAIDYX_API_TOKEN
...
if not path.startswith("/api/codex/"):
    print("refusing non-/api/codex path; use scoped Vaidyx integration endpoints only", file=sys.stderr)
    return 2
```
...
1. User opens Vaidyx Settings > Integrations and adds a Codex Agent.
2. User downloads the plugin bundle and registers it with Codex's plugin system:

```bash
curl -fsSL -H "Authorization: Bearer $VAIDYX_API_TOKEN" \
  "$VAIDYX_URL/api/codex/plugin.zip" -o /tmp/vaidyx-codex-plugin.zip
...
4. User runs `codex plugin add vaidyx@personal`.
...
```json
{
  "name": "vaidyx",
  "version": "0.1.1",
  "description": "Connect Codex to a scoped Vaidyx instance.",
  "skills": "./skills/",
  "interface": {
    "displayName": "Vaidyx",
    "shortDescription": "Use scoped Vaidyx tools from Codex.",
    "capabilities": ["todos", "email", "scoped-api"],
    "defaultPrompt": "Use Vaidyx only through configured scoped access. Check capabilities before reading or writing data."
  }
}
```
...
- Script paths reference `integrations/codex/scripts/vaidyx_api.py` or `~/plugins/vaidyx/scripts/vaidyx_api.py`.

### vaidyx_api.py (`integrations/codex/scripts/vaidyx_api.py`)

Identical implementation to the Claude version. Both files are 219 lines, same functions, same dispatch logic. The docstring reads "Small Vaidyx scoped API helper for Codex terminal sessions."
...
The companion system is a thin, additive LAN bridge that lets a mobile device (phone) discover and pair with an Vaidyx server, then use it for chat via scoped API tokens.
...
Both `integrations/claude/skills/vaidyx/scripts/vaidyx_api.py` and `integrations/codex/scripts/vaidyx_api.py` contain identical Python code. The helper:
