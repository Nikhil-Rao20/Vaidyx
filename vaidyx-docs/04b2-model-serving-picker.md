...
PID files in `$env:TEMP\vaidyx-sessions`.
...
2. Dispatches `vaidyx:model-picked` custom event.
...
default is configured. Caches the result in both `window.__vaidyxDefaultChat`
and `localStorage` key `vaidyx-default-chat-cache`.
...
`vaidyx-models-collapsed`). Only the first 5 models per endpoint are shown;
the rest behind a "Show N more" button.

### 4.3 Favorites and Usage Tracking

- **Favorites** (`vaidyx-model-favorites`): Toggle via star icon; favorited
  models appear in a dedicated top section.
- **Usage** (`vaidyx-model-usage`): Tracks `count` and `last` timestamp per
  model ID. Used by sort modes.
- **Sort modes** (`vaidyx-model-sort`): `alpha`, `last-used`, `most-used`,
