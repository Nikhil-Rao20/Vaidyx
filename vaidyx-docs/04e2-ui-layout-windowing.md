...
This document covers the Vaidyx frontend layout engine, modal/window management, theming, and interaction systems found in `static/js/`.
...
### Three-State Sidebar
`initSidebarLayout(Storage, opts)` (line 28) manages the sidebar in three modes persisted via `vaidyx-sidebar-mode` in localStorage:
...
- Dock position persistence via localStorage (`vaidyx.mobileDockState.v1`)
...
### Custom Themes
- Up to 8 custom themes stored in localStorage (`vaidyx-custom-themes`) and synced to server via `/api/prefs/custom-themes`.
