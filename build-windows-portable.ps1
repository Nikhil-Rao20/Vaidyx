...
<#
  Build a portable Windows distribution for Vaidyx.

  Output layout:
    dist\Vaidyx\Vaidyx.exe
    dist\Vaidyx\static\...
    dist\Vaidyx\scripts\...
    dist\Vaidyx\mcp_servers\...
    dist\Vaidyx\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Vaidyx @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Vaidyx" -ForegroundColor Green
