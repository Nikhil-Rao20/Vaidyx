...
def test_ollama_cookbook_runner_does_not_force_public_bind():
    ...
    assert 'OLLAMA_HOST="0.0.0.0:${VAIDYX_OLLAMA_PORT}" ollama serve' not in route
    assert 'OLLAMA_HOST="${VAIDYX_OLLAMA_HOST}:${VAIDYX_OLLAMA_PORT}" ollama serve' in route
...
def _load_search_content_for_test(monkeypatch, name="services.search.content_under_test"):
    ...
    cache.CONTENT_CACHE_DIR = Path("/tmp/vaidyx-test-content-cache")
