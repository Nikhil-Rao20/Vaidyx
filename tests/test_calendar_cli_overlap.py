"""Regression: `vaidyx-calendar list` must select events that OVERLAP the
...
`vaidyx-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "vaidyx-calendar"
    loader = importlib.machinery.SourceFileLoader("vaidyx_calendar_cli", str(path))
